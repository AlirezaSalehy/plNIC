...
namespace plNICDriver.Net.Fragmentation
{
	...
	{
		private static readonly int SEGMENT_MAX_LEN = Fragment.NUM_FIDS * Fragment.PAYLOAD_MAX_LEN;
		...
		private void CreateFragments(byte[] msg)
		{
			...
			{
				var midFrag = new Fragment(0, fragmentId, Sid,
												msg[(fragmentId * Fragment.PAYLOAD_MAX_LEN)..((fragmentId + 1) * Fragment.PAYLOAD_MAX_LEN)]);
				_fragments.Add(midFrag);
				fragmentId++;
				Console.WriteLine($"this is fragmentId: {fragmentId}");
			}
			var lastFrag = new Fragment(1, fragmentId, Sid,
		...
		public bool IsValid()
		{
			...
			for (var i = 0; i < lastFragFid; i++)
				if (_fragments.ElementAt(i).FID != i)
					return false;
