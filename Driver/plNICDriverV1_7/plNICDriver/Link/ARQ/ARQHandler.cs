...
namespace plNICDriver.Link.ARQ
{
	...
	public class ARQHandler
	{
		...
		public async Task<Link.Status> SendARQedFrame(Frame.FrameType ft, byte txId, byte rxId, byte[] payload)
		{
			if (ft == Frame.FrameType.SDU && payload.Length > 0)
			{
				var frees = _winElements.Where((winEl) => { return !winEl.filled; });
				if (!frees.Any())
					return Link.Status.Err_NoFreeWid;
				...
				return Link.Status.Err_GenFailure;
		}
