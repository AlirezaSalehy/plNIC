...
// provide pointers to. The 3rd argument should be the primes array to have it use
// the location specified above in this header.
void rsa_gen_keys(RSAKey_t *pub, RSAKey_t *priv,
						const uint32_t* ps, const size_t psLen, const uint16_t seed);
...
long long *rsa_encrypt(const char *message, const unsigned long message_size, const RSAKey_t *pub);
...
char *rsa_decrypt(const long long *message, const unsigned long message_size, const RSAKey_t *pub);
